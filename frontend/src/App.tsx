...
function App() {
  type Tab = 'chat' | 'pipelines' | 'projects'

  const [activeTab, setActiveTab] = useState<Tab>(() => {
    if (typeof window === 'undefined') return 'chat'
    const stored = window.localStorage.getItem('workforce-active-tab')
    if (stored === 'pipelines' || stored === 'projects' || stored === 'chat') {
    ...
    <div className="h-screen w-full flex flex-col bg-background">
      <header className="border-b border-border bg-card px-4 py-2 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] text-muted-foreground">Chat, Pipelines & Projects</span>
        </div>
